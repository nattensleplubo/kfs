...
void    print_hex(unsigned int value, int size) {
...
void    print_stack(uint32_t stack_top, int limit) {
            ...
            if (i != 0) {
                ...
                for (int y = 0; y < 16; y++) {
                    if (start_point[y] <= 32 || start_point[y] >= 126)
                        ...
                        terminal_putchar(start_point[y]);
                }
                terminal_putchar('\n');
            }
            terminal_putstr("0x");
            print_hex((unsigned int)stack_top, 8);
            terminal_putstr(": ");
        }
        terminal_putchar(' ');
        print_hex(*address, 2);
...
void process_command() {
    ...
    if (strcmp(command_buffer, "help") == 0)
        ...
        terminal_putchar('\n');
        print_stack((uint32_t)esp - 256, ebp);
    }
    else {
        terminal_putstr("\nUnknown command: ");
